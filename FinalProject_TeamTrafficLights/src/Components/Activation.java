...
public class Activation implements Serializable {
	...
	public void Activate() throws CloneNotSupportedException {
		...
		if (Operation == TransitionOperation.Factorial)
			Factorial();
...
//			PetriObject temp = Parent.TempMarking.get(inputIndex);
	...
	private void Factorial() throws CloneNotSupportedException {
		Integer outputIndex = util.GetIndexByName(OutputPlaceName, Parent.Parent.PlaceList);
		PetriObject temp = util.GetFromListByName(InputPlaceName, Parent.TempMarking);
		PetriObject result = null;
		...
		if (temp instanceof DataInteger) {
			result = (PetriObject) ((DataInteger) temp).clone();
			Integer userValue = (Integer) result.GetValue();
			if (userValue == 0 || userValue == 1) {
				result.SetValue(1);
			} else if (userValue > 0) {
				Integer res = 1;
				for (Integer i = 2; i <= userValue; i++) {
					res *= i;
					result.SetValue(res);
				}
			}else {
		...
		if (temp instanceof DataFloat) {
			result = (PetriObject) ((DataFloat) temp).clone();
			Float userValue = (Float) result.GetValue();
			if (userValue == 0.0f || userValue == 1.0f) {
				result.SetValue(1.0f);
			} else if (userValue > 0.0f) {
				Float res = 1.0f;
				while (userValue > 0.0f) {
					res *= userValue;
					userValue -= 1.0f;
				}
				result.SetValue(res);
